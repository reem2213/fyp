...
const Goal = ({ navigation ,route}) => {
  ...
  return (
    <View style={[styles.container, isDarkMode && styles.darkContainer]}>
      ...
      <Image source={GoalScreen} style={styles.goalImage} />
      <Text style={[styles.GoalTracking, isDarkMode && styles.darkText]}>Goal Tracking</Text>
      <Modal
        visible={isModalVisible}
        animationType="slide"
        transparent={true}
      ...
      >
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            ...
            <Button title="Pick a Date" onPress={showDatePicker} />
            {selectedDate && (
              <Text style={[styles.selectedDate, isDarkMode && styles.darkText]}>
                Selected Date: {selectedDate.toLocaleDateString()}
              </Text>
            )}
            <Button title="Add Goal" onPress={handleSaveGoal} />
      ...
      <GestureHandlerRootView style={{ height: "40%" }}>
        <ScrollView style={styles.goalsContainer} showsVerticalScrollIndicator>
          {goals.map((goalItem, index) => (
            <View key={index} style={[styles.goalItemContainer, isDarkMode && styles.darkGoalItemContainer]}>
              <Text style={[styles.goalItem, isDarkMode && styles.darkText]}>{goalItem.goal}</Text>
              <Pressable onPress={() => handleMoveGoal(goalItem)}>
                <Image
                  source={TrashIcon}
                  style={{ width: 30, height: 30, marginRight: 20 }}
      ...
      <Pressable style={[styles.add, isDarkMode && styles.darkAdd]} onPress={handleAddGoal}>
        <Text style={styles.addGoal}>+ Add a new Goal</Text>
      </Pressable>
